...
void Mod::lazyLoadSurface(const std::string &name, int width, int height)
{
	if (Options::lazyLoadResources)
	{
		...
		if (i != _extraSprites.end())
		{
			...
			{
				loadExtraSprite(extraSprites, width, height);
			}
		}
	}
}
...
void Mod::ScriptRegister(ScriptParserBase *parser)
{
	...
	mod.add<&offset<&Mod::_surfaceOffsetBigobs32>>("getSpriteOffsetBigobs32", "convert mod surface index in first argument to runtime index in given set, second argument is mod id");
